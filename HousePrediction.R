...
head(london) #dataset for our multiple linear regression to predict london house prices

plot(london$Price, london$Area.in.sq.ft, xlab = "Prices", ylab = "Area per sq " , main = "London house prices with total area per sq", col="blue")

pairs(london[,1:6])

round(cor(london[,3:6], method = "pearson"), 2)

rcorr(as.matrix(london[,3:6]))

pricePrediction <- lm(Price ~ No..of.Bedrooms+Area.in.sq.ft, data = london)

summary(pricePrediction)

#Let's predict our price if number of bedrooms are 7 and area in sq.ft is 1500
newMulti <- data.frame(No..of.Bedrooms=5, Area.in.sq.ft=1500)

predict(pricePrediction, newMulti)#our price will be 972359.3

conInter <- predict(pricePrediction, newMulti, interval = "confidence", level = 0.95)
print(conInter)
...
index <- createDataPartition(lassDF$Price, p=.8, list = FALSE, times= 1)
#Create test and trining data 
train_lassDF <- lassDF[index,]

test_lassDF <- lassDF[-index,]

ctrlspecs <- trainControl(method = "cv", number = 10,
                          savePredictions = "all")


#Specify and train lasso regression

#Create vector of lambda potential
lambda_vector <- 10^seq(5, -5, length= 500)

set.seed(2000)

model1 <- train(Price ~ Area.in.sq.ft+No..of.Bedrooms+No..of.Bathrooms, 
                data= train_lassDF,
...
model1$bestTune
model1$bestTune$lambda#13129.15

#Lasso regression model coefficents 
...
predictions1 <- predict(model1, newdata = test_lassDF)

#Model performance/Accuracy
mod1perf <- data.frame(RMSE=RMSE(predictions1, test_lassDF$Price),
                       RSquared = R2(predictions1, test_lassDF$Price))

#Seting seed
set.seed(2000)

#Comparing OLS multiple linear regression model to Lasso regression 
model2 <- train(Price ~ Area.in.sq.ft+No..of.Bedrooms+No..of.Bathrooms,
                data = train_lassDF,
                preProcess = c("center", "scale"),
                method = "lm",
                trControl = ctrlspecs,
...
#Coefficent of Model2
model2$finalModel$coefficients

#Compare model performances of k-fold cross validation on train_lassDF (comparing Lasso regression and OLS multiple linear regression)
model_list <- list(model1, model2)
resamp <- resamples(model_list)
summary(resamp)

#Comparing model using paired-sample t-test
compare_models(model1, model2, metric = "Rsquared")

predictions2 <- predict(model2, newdata = test_lassDF)

#model performance
mod2Perf <- data.frame(RMSE=RMSE(predictions2, test_lassDF$Price),
                       RSquared = R2(predictions2, test_lassDF$Price))


print(mod2Perf)

#Comparing model1 and model2 predictive performance based on test_lassDF
comp <- matrix(c(mod1perf$RMSE, mod1perf$RSquared,
                 mod2Perf$RMSE, mod2Perf$RSquared),
               ncol = 2, byrow = TRUE)

colnames(comp) <- c("RMSE", "Rsquared")

rownames(comp) <- c("LassoRegression", "OLS Multiple linear regression")

comp <- as.table(comp)
round(comp, 2)


                            #Simple Linear regression for predicting dublin house prices

dublinHouse <- read.csv("C:/Users/umeri/Videos/MSc in Data Analytics/DataMining and Machine Learning 1/DMML1/dublinDf.csv")
View(dublinHouse)

summary(dublinHouse)

cor(dublinHouse$houseBeds, dublinHouse$housePrice)

housePred <- lm(housePrice ~ houseBeds , data = dublinHouse)
housePred

summary(housePred)

newHousePrice <- data.frame(houseBeds=3)

predict(housePred, newHousePrice)#our price will be 342218.1 
